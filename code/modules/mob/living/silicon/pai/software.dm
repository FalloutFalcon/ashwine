	...
	if(soft)
		...
		switch(soft)
			...
			if("medicalrecord") // Accessing medical records
				if(subscreen == 1)
					medicalActive1 = SSdatacore.find_record("id", href_list["med_rec"], GLOB.data_core.general)
					if(medicalActive1)
						medicalActive2 = SSdatacore.find_record("id", href_list["med_rec"], GLOB.data_core.medical)
					if(!medicalActive2)
						medicalActive1 = null
			...
			if("securityrecord")
				if(subscreen == 1)
					securityActive1 = SSdatacore.find_record("id", href_list["sec_rec"], GLOB.data_core.general)
					if(securityActive1)
						securityActive2 = SSdatacore.find_record("id", href_list["sec_rec"], GLOB.data_core.security)
					if(!securityActive2)
